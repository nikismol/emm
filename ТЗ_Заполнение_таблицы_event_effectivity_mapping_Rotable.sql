/* *******************************************************
 * *** TO DO
 * *** 1. Доработать если в эффективити указано несколько раз один
   и тот же партийный номер
******************************************************* */

/* *******************************************************
 * *** ТЗ: Заполнение таблицы event_effectiviry_mapping
 * *** SQL - скрипт	S1
******************************************************* */
...
CREATE TABLE схемаEVENT_EFFECTIVITY_MAPPING_ROTABLES_TEST
(
...
CREATE TABLE схемаdocument_status (
    code VARCHAR2(5),
    name VARCHAR2(100),
    description CLOB,
...
CREATE OR REPLACE PROCEDURE схемаprepare_test_area_ro (
...
BEGIN
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE схемаevent_effectivity_mapping_rotables_test';
    EXECUTE IMMEDIATE 'TRUNCATE TABLE схемаdocument_status';

    /* Таблица event_effectivity_mapping_tes заполняется данными при активации новой ревизии в effectivity */
  INSERT INTO схемаEVENT_EFFECTIVITY_MAPPING_ROTABLES_TEST
  SELECT
    'EFL',
    doc_header.docno_i,
    ...
    'N',
    aircraft.ac_registr,
    COALESCE(aircraft.aircraftno_i, 0),
    rotables.psn,
    '',
    '',
    0,
  ...
  FROM схемаdoc_header
  LEFT JOIN схемаevent_effectivity_link ON doc_header.docno_i = event_effectivity_link.event_key AND event_effectivity_link.event_type = 'DO'
  LEFT JOIN схемаevent_effectivity ON event_effectivity_link.effectivityno_i = event_effectivity.effectivityno_i
  LEFT JOIN схемаapplicability ON event_effectivity.effectivityno_i = applicability.effectivityno_i AND applicability.ref_type = 'RO'
  LEFT JOIN схемаrotables ON applicability.ref_key = rotables.psn
  LEFT JOIN схемаaircraft ON rotables.ac_registr = aircraft.ac_registr
  ...
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES (' ', ' ', ' ', '-', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('!', 'Unknown', 'Unknown', '-', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('-', 'Not Assigned', '-', '-', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('/', 'No status / Not installed', 'Component not installed into this Aircraft', '-', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('0', 'Not performed', '-', 'Open', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('1', 'Aircraft group changed', '-', '-', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('2', 'Aircraft/Rotable deactivated', '-', 'N/A', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('3', 'Effectivity Historized', '-', 'N/A', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('?', 'No status', '-', '-', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('A', 'On Attrition', 'Indicates that the modification of the rotable is only done in the course of a repair when the unit is removed and sent off, comparable to a maintenance or soft-time limit of a rotable requirement. You would not remove a rotable just for the performance of this document. Status: open. Before reporting back the document, it is possible to change the assignment code to "open" again. Reporting back a document with the status On Attrition, the document can be initialised in Maintenance Event Initialiser and in View/Edit Modifications.', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('B', 'Postponed (open)', 'The performance of the document has been postponed. Status: ope', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('C', 'Closed', 'Closed by your company. The normal closing procedure is done in Reporting Back Maintenance Event', 'Closed', 'Post');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('D', 'Delete assignment!', 'When you need to change the assignment status of an aircraft or component you must delete the assignment first before you can assign a new status.', '-', '-');
  ...
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('F', 'Ex Factory', 'The modification has already been performed by the manufacturer. Status: closed.', 'Closed', 'Post');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('G', 'Term.Action open', '-', 'Open', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('H', 'Rotable Driven - Accomplished', '-', 'Closed', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('I', 'Information Only', 'No status.', '-', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('J', 'Rejected by Operator', 'Status: open.', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('K', 'Rotable Driven - Not Accomplished', '-', 'Open', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('L', 'Logbook Controlled', 'Status: open. This document does not appear in the forecast.', 'Open', 'N/A');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('M', 'Taskcard Controlled', 'A repetitive document might be controlled by a taskcard. Status: open. This document does not appear in the forecast.', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('N', 'Not Applicable', 'Can also be set manually. You must enter a reason for this status in the Initialise Documents window. This reason will also be printed on the Modification Status report.', 'N/A', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('O', 'Open', 'Open.', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('P', 'Partly Performed', '-', 'Open', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('Q', 'Part-Requirem.Contr.', 'A repetitive document might be controlled by a part requirement. Status: open. This document does not appear in the forecast.', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('R', 'Repetitive', '-', 'Open', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('S', 'DOC.IN PREP.', '-', 'Open', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('T', 'Terminated by third party', 'The document has been performed by a third party. Status: closed.', 'Closed', 'Post');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('U', 'Cancelled', 'This status is automatically set for documents which were cancelled or superseded - see Replacement Information section.' ||
  ...
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('V', 'Performed by prev. Operator', 'The document has been performed by the previous operator of the aircraft or component. Status: closed.', 'Closed', 'Post');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('W', 'See prev. Doc. Status', 'This status indicates that the status of the document has not been entered into AMOS. This status is highlighted orange in View/Edit Modifications.', '-', 'N/A');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('X', 'Externally Controlled', '-', 'N/A', '-');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('Y', 'Reopened', 'Status of the document is open. Setting the status to reopen is only possible if the document was closed before. When setting this status, a reason can be given.', 'Open', 'Pre');
  INSERT INTO схемаdocument_status (code, name, description, status_group, pre_post) VALUES ('Z', 'Superseded', 'This status is automatically set for documents which were cancelled or superseded - see Replacement Information section.' ||
                                                                                                         'It is also possible to assign this status manually to individual serial numbers. You must enter a reason for this status in the Initialise Documents window. This reason will also be printed on the Modification Status report.', 'N/A', 'Pre');
...
CREATE OR REPLACE PROCEDURE схемаupdate_status_tree_row_doc_rot (
        p_docno_i NUMBER, p_psn NUMBER
)
IS
    ...
    WHERE sub.root=cp_docno_i AND applicability.ref_key=cp_psn
      AND parrent.docno_i<>sub.docno_i;
BEGIN
    OPEN c_update_status_tree (p_docno_i, p_psn);
    LOOP
        FETCH c_update_status_tree  INTO
            v_parrent_docno_i,
            v_psn;
        EXIT WHEN c_update_status_tree%NOTFOUND;
        UPDATE схемаevent_effectivity_mapping_test
            SET needs_recalculation='Y'
            WHERE event_key_parent=v_parrent_docno_i AND psn=v_psn;
    END LOOP;
    COMMIT;
...
CREATE OR REPLACE FUNCTION схемаGET_EVENT_STATUS_RO(
    p_docno_i IN NUMBER,
    p_psn IN NUMBER
)
    RETURN VARCHAR2
    as event_status VARCHAR2(1);
        BEGIN
            ...
            FROM (
                ...
                JOIN event_effectivity_link ON parrent.docno_i = event_effectivity_link.event_key AND event_effectivity_link.event_type = 'DO'
                JOIN event_effectivity ON event_effectivity_link.effectivityno_i = event_effectivity.effectivityno_i
                JOIN applicability ON event_effectivity.effectivityno_i = applicability.effectivityno_i AND applicability.ref_type='RO'
            ...
            JOIN applicability ON event_effectivity.effectivityno_i = applicability.effectivityno_i
                                      ...
                                      AND applicability.ref_type = 'RO'
            JOIN rotables ON applicability.ref_key=rotables.psn
            LEFT JOIN mevt_header ON applicability.ref_key = mevt_header.ref_key
                                         AND mevt_header.ref_type='RO'
            ...
            WHERE doc_sign_off.docno_i = p_docno_i AND doc_sign_off.ref_key = p_psn
            GROUP BY doc_sign_off.docno_i, rotables.psn) status_event;
        RETURN event_status;
    END;
...
CREATE OR REPLACE FUNCTION схемаGET_EVENT_PERFNO_I_RO(
    p_docno_i IN NUMBER,
    p_psn IN NUMBER
)
    RETURN NUMBER
...
CREATE OR REPLACE FUNCTION схемаGET_LAST_EVENT_PERFNO_I_RO(
    p_docno_i IN NUMBER,
    p_psn IN NUMBER
)
    RETURN NUMBER
            ...
            FROM(
                ...
                FROM (
                    ...
                    FROM (
                        ...
                        JOIN event_effectivity ON event_effectivity_link.effectivityno_i = event_effectivity.effectivityno_i
                        JOIN applicability ON event_effectivity.effectivityno_i = applicability.effectivityno_i AND applicability.ref_type='RO'
                        LEFT JOIN mevt_header ON mevt_header.ref_key=applicability.ref_key
                                                     AND parrent.docno_i = mevt_header.mevt_key
                                                     AND mevt_header.mevt_type = 'DO'
                        LEFT JOIN mevt_effectivity ON mevt_header.mevt_headerno_i = mevt_effectivity.mevt_headerno_i
                        WHERE pid IS NOT NULL
...
CREATE OR REPLACE FUNCTION схемаGET_EVENT_STATUS_SINGLE_RO(
    p_docno_i IN NUMBER,
    p_psn IN NUMBER
)
    RETURN VARCHAR2
...
BEGIN
    ...
    FROM (
        ...
        LEFT JOIN document_status ON wo_event_link.event_status=document_status.code
        WHERE doc_sign_off.docno_i = p_docno_i AND doc_sign_off.ref_key = p_psn
        GROUP BY doc_sign_off.docno_i, rotables.psn
        ) single_status_event;
...
CREATE OR REPLACE PROCEDURE update_event_mapping_ro (
    p_docno_i        IN NUMBER,
    p_psn            IN NUMBER
)
IS
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(NULL, 'N'),
        ac.ac_registr,
        COALESCE(ac.aircraftno_i, 0),
        r.psn,
        схемаGET_EVENT_STATUS_RO(dh.docno_i, r.psn),
        схемаGET_EVENT_STATUS_SINGLE_RO(dh.docno_i, r.psn),
        схемаget_event_perfno_I_RO(dh.docno_i, r.psn),
        схемаGET_LAST_EVENT_PERFNO_I_RO(dh.docno_i, r.psn),
        0,
    ...
    LEFT JOIN applicability app ON ef.effectivityno_i = app.effectivityno_i AND app.ref_type = 'RO'
    LEFT JOIN rotables r ON app.ref_key = r.psn
    LEFT JOIN aircraft ac ON r.ac_registr=ac.ac_registr
    WHERE dh.docno_i = p_docno_i
      AND ef.status = 0
      AND app.ref_key = p_psn;

    -- Обновляем целевую таблицу
    UPDATE схемаevent_effectivity_mapping_rotables_test eem
    SET
...
BEGIN
    схемаprepare_test_area_ro(p_docno_i => :docno_i, p_psn => :psn);
    COMMIT;
END;

BEGIN
    update_event_mapping_ro(p_docno_i => :docno_i, p_psn => :psn);
    COMMIT;
END;



CREATE OR REPLACE VIEW схемаevent_effectivity_mapping_test_rotables_check AS
    SELECT r.*
    FROM
        (
            SELECT
            ...
            FROM схемаevent_effectivity_mapping_rotables_test r
            WHERE r.event_key_parent=:docno_i AND r.psn=:psn
            UNION
            SELECT
                ...
                event_perfno_i,
                effectivity_linkno_i
            FROM схемаevent_effectivity_mapping r
            WHERE r.event_key_parent=:docno_i AND r.psn=:psn
            ) r
...
SELECT
    *
FROM event_effectivity_mapping_test_rotables_check